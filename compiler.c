...
static void loopSwitchCase(int start) {
  consume(TOKEN_CASE, "Expect 'case' after switch.");
  expression();
  consume(TOKEN_COLON, "Expect ':' after case value.");
...
static void loopSwitchStatement() {
  int loopStart = currentChunk()->count;
  consume(TOKEN_LEFT_PAREN, "Expect '(' after 'loop'.");
  expression();
  consume(TOKEN_RIGHT_PAREN, "Expect ')' after loop condition.");
  consume(TOKEN_LEFT_BRACE, "Expect '{' after loop condition."); 

  while (!check(TOKEN_RIGHT_BRACE) && !check(TOKEN_EOF) && !check(TOKEN_DEFAULT)) {
    loopSwitchCase(loopStart);
  }
  ...
  emitByte(OP_POP);
  consume(TOKEN_RIGHT_BRACE, "Expect '}', after loop.");
}
...
static void statement() {
  if (match(TOKEN_PRINT)) {
    ...
    switchStatement();
  } else if (match(TOKEN_LOOPSWITCH)) {
    loopSwitchStatement();
